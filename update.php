...
if (isset($_SESSION['key'])) {
    if (@$_GET['dusername'] && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        ...
        $result = mysqli_query($con, "DELETE FROM user_answer WHERE username='$dusername' ") or die('Error');
...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        ...
        $status  = "disabled";
        $id      = uniqid();
        $q3      = mysqli_query($con, "INSERT INTO quiz VALUES(NULL,'$id','$name','$practice_op','$correct','$wrong','$total','$time', 'NOW()','$status')");
        header("location:dash.php?q=4&step=2&eid=$id&n=$total");
    }
}
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addquiz_challenge' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        $name    = $_POST['name'];
        $image_link    = $_POST['image_link'];
        $quiz_desc   = $_POST['quiz_desc'];
        $date_comp   = $_POST['date_comp'];
        $time_quiz   = $_POST['time_quiz'];
        $difficulty   = $_POST['difficulty'];
       
        $q3      = mysqli_query($con, "INSERT INTO quiz_challenge_comp(`name`, `image_link`, `quiz_desc`, `date_comp`, `time_quiz`, `difficulty`) VALUES('$name','$image_link','$quiz_desc','$date_comp','$time_quiz','$difficulty')");
        if(mysqli_query( $q3)){
            echo "Records added successfully.";
        } else{
            echo "ERROR: Could not able to execute $q3.";
        }
